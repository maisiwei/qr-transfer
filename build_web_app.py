    ...
    <div id="panel-send" class="panel active">
        ...
        <div class="qr-display-container">
            ...
            <div id="sender-camera-panel" style="margin-top: 15px; display: flex; flex-direction: column; align-items: center; gap: 8px; width: 100%;">
                <button type="button" id="btn-toggle-sender-cam" class="btn" style="padding: 10px 16px; font-size: 0.85rem; background: var(--bg-base); border: 1px solid var(--border-card);" onclick="openSenderCamModal()">
                    <div id="sender-cam-status-dot" style="width: 8px; height: 8px; border-radius: 50%; background: #94a3b8; display: inline-block; margin-right: 8px; transition: background 0.3s;"></div>
                    <span id="sender-cam-btn-text">Enable Feedback Camera</span>
                </button>
            </div>
        </div>
    </div>
...
<script>
    ...
    function openSenderCamModal() {{
        document.getElementById('sender-camera-modal').style.display = 'flex';
        startSenderCam();
    }}

    function closeSenderCamModal() {{
        document.getElementById('sender-camera-modal').style.display = 'none';
        stopSenderCam();
    }}
    ...
    function startSenderCam() {{
        ...
        navigator.mediaDevices.getUserMedia(constraints)
            .then(function(stream) {{
                ...
                document.getElementById('sender-cam-btn-text').innerText = "Disable Feedback Camera";
    ...
    function flipSenderCamera() {{
        if (senderCamIsActive) {{
            ...
            // Wait 250ms for iOS Safari to release the camera hardware resource completely
            setTimeout(function() {{
                navigator.mediaDevices.getUserMedia(constraints)
                    .then(function(stream) {{
                        senderCamStream = stream;
    ...
    function stopSenderCam() {{
        ...
        document.getElementById('sender-cam-btn-text').innerText = "Enable Feedback Camera";
    ...
    function changeCamera() {{
        if (cameraStream) {{
            // Restart camera with new ID, preserving current scan progress
            stopScanner();
            startScanner(true);
        }}
    }}
    ...
    function toggleScanner() {{
        ...
        if (cameraStream) {{
            ...
            startScanner(true); // Preserve scan progress when toggling/resuming
    ...
    function showMissingChunksQR() {{
        ...
        if (missingArray.length > 0) {{
            ...
            // Open receiver feedback modal
            document.getElementById('receiver-feedback-modal').style.display = 'flex';
        }}
    }}

    function resumeReceiverScanning() {{
        document.getElementById('receiver-feedback-modal').style.display = 'none';
...
<!-- Sleek Modern Modal Overlays -->
<div id="sender-camera-modal" class="modal-overlay">
    <div class="modal-content">
        <div class="modal-header">Feedback Camera Link</div>
        <div style="font-size: 0.85rem; color: var(--text-muted); text-align: center; margin-top: -8px;">
            Point your camera at the receiver's screen to capture missing chunks.
        </div>
        
        <div id="sender-camera-controls" style="display: none; margin-bottom: 5px; width: 100%; gap: 6px;">
        ...
        <div id="sender-video-container" style="position: relative; width: 100%; height: 210px; border-radius: 12px; overflow: hidden; border: 1px solid var(--border-card); background: #000; display: none;">
            <video id="sender-video" style="width: 100%; height: 100%; object-fit: cover;" autoplay playsinline muted></video>
            <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; box-shadow: inset 0 0 15px rgba(16, 185, 129, 0.4); pointer-events: none; border: 2px solid #10b981; border-radius: 12px; box-sizing: border-box; transition: all 0.3s;" id="sender-video-border"></div>
        </div>
        
        <button class="btn btn-secondary" style="width: 100%; border: none;" onclick="closeSenderCamModal()">Close Camera</button>
    </div>
</div>

<div id="receiver-feedback-modal" class="modal-overlay">
    <div class="modal-content">
        <div class="modal-header">Scan Missing Chunks</div>
        <div style="font-size: 0.85rem; color: var(--text-muted); text-align: center; margin-top: -8px;">
            Point this code at the sender's camera to request the missing chunks.
        </div>
        ...
        <div style="font-size: 0.8rem; color: var(--text-muted); text-align: center;" id="feedback-desc">Requesting missing chunks...</div>
        
        <button class="btn" style="width: 100%; border: none;" onclick="resumeReceiverScanning()">Resume Scanning</button>
    </div>
</div>
