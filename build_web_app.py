...
<div class="container">
    ...
    <div id="panel-receive" class="panel">
        ...
        <div id="receiver-feedback-card" style="display: none; flex-direction: column; align-items: center; gap: 10px; margin-top: 15px; padding: 15px; background: var(--bg-card); border-radius: 12px; border: 1px solid var(--border-card); width: 100%; box-sizing: border-box;">
            <div style="font-size: 0.85rem; font-weight: 600; color: var(--text-main);">Point this code at MacBook's webcam</div>
...
<script>
    ...
    function finishScanning() {{
        ...
        // Render FB:COMPLETE feedback code so the MacBook can confirm success
        showCompleteFeedbackQR();
    ...
    function showMissingChunksQR() {{
        ...
        var payload = "FB:COMPLETE";
        if (missingArray.length > 0) {{
            var compressed = compressMissingIndices(missingArray);
            payload = "FB:" + compressed;
            document.getElementById('feedback-desc').innerText = "Requesting " + missingArray.length + " missing chunks.";
        }} else {{
            document.getElementById('feedback-desc').innerText = "Transfer Complete! Show this to MacBook camera.";
        }}
        
        var hasMissing = (missingArray.length > 0);
        document.querySelector('#receiver-feedback-card button').style.display = hasMissing ? 'block' : 'none';
    ...
    function showCompleteFeedbackQR() {{
        if (!feedbackQrGenerator) {{
            feedbackQrGenerator = new QRCode(document.getElementById("feedback-qrcode"), {{
                text: "Init",
                width: 140,
        ...
        feedbackQrGenerator.clear();
        feedbackQrGenerator.makeCode("FB:COMPLETE");
        
        document.getElementById('feedback-desc').innerText = "Transfer Complete! Show this to MacBook camera.";
        document.querySelector('#receiver-feedback-card button').style.display = 'none';
        document.getElementById('receiver-feedback-card').style.display = 'flex';
    }}
